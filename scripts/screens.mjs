...
const ServingPicker = (await load('/src/components/ServingPicker.jsx')).default;
const { defaultServing } = await load('/src/api/portion.js');
...
const eggFood = { name: 'Egg, Raw', detail: 'USDA Foundation' };
...
const screens = {
  ...
  // The portion picker, in the two states that matter: a food we have real
  // sizes for, and one we don't (where it must render nothing at all).
  servings: React.createElement('div', { className: 'sheet-backdrop' },
    React.createElement('div', { className: 'sheet' },
      ...
      React.createElement('div', { className: 'sheet-head' },
        React.createElement('h2', null, 'Add by hand'),
        React.createElement('button', { className: 'link-button' }, 'Close')),
      React.createElement('label', { className: 'stack-tight' },
        React.createElement('span', { className: 'label muted' }, 'What is it'),
        React.createElement('input', { readOnly: true, defaultValue: 'Eggs' })),
      React.createElement('div', { className: 'add-product' },
        React.createElement('div', { className: 'stack-tight' },
          React.createElement('span', null, 'Egg, Raw'),
          React.createElement('span', { className: 'label muted' }, 'USDA Foundation')),
        React.createElement('button', { className: 'link-button' }, 'Change')),
      React.createElement('div', { className: 'add-qty is-picker' },
        React.createElement(ServingPicker, {
          food: eggFood, quantity: 12,
          serving: defaultServing(eggFood), onQuantity() {}, onServing() {}
        }),
        React.createElement('label', { className: 'stack-tight' },
          React.createElement('span', { className: 'label muted' }, 'Where'),
          React.createElement('select', { defaultValue: 'fridge' },
            React.createElement('option', { value: 'fridge' }, 'fridge')))),
      React.createElement('button', { className: 'primary' }, 'Add it'))),
